...
<style>
    ...
    .stTabs [data-baseweb="tab"] {background-color: #f0f2f6; border-radius: 4px 4px 0 0; padding: 10px 20px; font-weight: bold;}
    .stTabs [aria-selected="true"] {background-color: #ff4b4b; color: white;}
</style>
""", unsafe_allow_html=True)

st.title("🧬 跨领域材料本征与系统级全景推演引擎 (v12.0 终极自适应版)")
st.caption("动态领域表单 | 本征对标矩阵 | 数学等效推演 | 图表化扫掠 | 六维图文切片 | 段落小结与终审决议")
...
if st.button("🚀 启动跨领域自适应全景引擎 (预计20-40秒)", type="primary"):
    if not api_key:
    ...
    system_prompt = f"""
    你是跨领域材料全生命周期推演引擎。
    输入：领域={domain}, 类别={mat_category}, 形态={material_form}, 密度={density}, 强度={strength}, 模量={modulus}, 伸长率={elongation}%, 附加参数={extra_context}。
    
    【核心要求】
    1. 动态自适应：如果领域涉及生物医疗/降解，图表和评价必须重点偏向毒理、降解速率、孔隙率等；如果是机械臂，偏向刚度与疲劳。
    2. 继承机制：必须包含数学公式(LaTeX)、雷达图、扫掠图表。
    3. 模块小结：每部分的 "section_summary" 必须给出带数据的结论。
    4. 选项卡带图：在 "six_dimensions" 中，每个维度必须提供 chart_metric, base_val, new_val 用于渲染内部条形图。
    
    必须严格输出此庞大的 JSON 结构（严禁包含Markdown标记）：
    {{
      "radar": {{"绝对强度/活性": 100, "刚度/支撑性": 60, "轻量化/孔隙率": 95, "加工/相容性": 45, "疲劳/降解周期": 50, "环境抗性": 60}},
      "base_metrics": [
      ...
      "summary_1": "本征对标小结：(基于数据的一句话总结)",
      
      "math_sim": {{
        "part_name": "典型核心部件 (根据领域推演)",
        "design_goal": "核心等效代换目标",
        "math_latex": "在此写出力学或降解速率等效公式，如 $$ \\delta = \\frac{{F L^3}}{{3 E I}} $$ 或阿伦尼乌斯方程，展示 $E={modulus}$ 等参数的推演过程。",
        "table": [
          {{"param": "核心代换指标", "base": "基准值", "new": "新材料计算值"}},
          {{"param": "次级参数演变", "base": "基准值", "new": "新材料计算值"}},
          {{"param": "最终总重/总效能", "base": 4.25, "new": 1.35}}
        ],
        "chart_vals": {{"base_wt": 4.25, "new_wt": 1.35}}
      }},
      "summary_2": "等效推演小结：(带百分比的数据总结)",

      "parameter_sweep": {{
        "sweep_1": {{
          "chart_title": "韧性/降解率随参数波动预估",
          "chart_data": [{{"x": "下限预估", "y": 20}}, {{"x": "中位理论值", "y": 120}}, {{"x": "上限风险", "y": 40}}],
          "scenarios": [
            {{"range": "区间1", "desc": "工程后果1"}},
            {{"range": "区间2", "desc": "工程后果2"}}]
        }},
        "sweep_2": {{
          "chart_title": "湿热/生化环境衰减曲线",
          "chart_data": [{{"x": "常规环境", "y": 100}}, {{"x": "严苛环境1", "y": 70}}, {{"x": "极限挑战", "y": 20}}],
          "scenarios": [
            {{"range": "环境预警1", "desc": "工程后果1"}},
            {{"range": "环境预警2", "desc": "工程后果2"}}]
        }}
      }},
      "summary_3": "敏感性扫掠小结：(揭示最大风险盲区)",
      ...
      "six_dimensions": [
        ...
        {{"dim": "6. 整机经济与降本效益", "details": ["数据点1", "数据点2"], "chart_metric": "整机综合BOM降本", "base_val": 0, "new_val": 15}}
      ],
      "summary_4": "全维切片小结：(六个维度综合评估的短板效应)",
      ...
      "grand_verdict": {{
        "summary": "最终工程结案陈词：基于以上所有数据的几百字宏观定调，必须包含具体数据和场景限制。",
    ...
    with st.spinner("算力引擎全开：加载动态矩阵 -> 数学仿真 -> 区间扫掠 -> 内部图表生成 -> 终极闭环汇总..."):
        try:
            ...
            st.success("✅ 终极自适应数据矩阵推演完成！")

            # ================= I. 基础矩阵 (雷达+微缩图+表+小结) =================
            st.subheader("I. 领域自适应本征对标矩阵")
            ...
            with sc2:
                df_wt = pd.DataFrame({"方案": ["传统基准", "新材料方案"], "最终效能数值": [sim['chart_vals']['base_wt'], sim['chart_vals']['new_wt']]})
                fig_wt = px.bar(df_wt, x="方案", y="最终效能数值", color="方案", text="最终效能数值", height=200,
            ...
            # ================= IV. 六维图文切片 =================
            st.subheader("IV. 工程师专属六维切片 (内嵌靶向图表)")
            st.markdown("👉 **请点击下方不同维度的标签页，查看专属图表与数据分析**")
            
            dims = data['six_dimensions']
            # 使用醒目的编号和 Emoji 强化 Tab
            ...
            with bot_c1:
                ...
                img_url = f"https://placehold.co/600x300/1e293b/e2e8f0?text={urllib.parse.quote(domain + ' Application')}"
            ...
            with bot_c2:
                ...
                # 终极宏观总结
                st.success(f"**🏆 终审结论：** {verdict['summary']}")

...
# ================= 🌟 核心：全域知识图谱配置字典 🌟 =================
# 只要在这里配置好，UI 界面、搜索逻辑、AI 脑回路会自动全量适配！
DOMAIN_CONFIG = {
    "生物医疗与人体植入物 (生化导向)": {
        "ui_inputs": [
        ...
        "search_suffix": "biocompatibility ISO 10993 in vivo degradation rate medical implants",
    ...
    "军工：单兵装甲与防护装备 (防弹/吸能)": {
        ...
        "search_suffix": "ballistic impact resistance body armor energy absorption MIL-STD",
    ...
    "人形机器人核心骨架 (高动载/刚度)": {
        ...
        "search_suffix": "dynamic mechanics vibration damping lightweight humanoid robot frame",
    ...
    "航空航天与eVTOL飞行器 (极致轻量化)": {
        ...
        "search_suffix": "aerospace structural materials thermal stability FAA regulations",
        "strict_constraint": "【绝对禁令】：严禁提及医疗或普通民用标准！重点分析高低温循环交变应力、抗微陨石冲击及适航认证(FAA/EASA)。"
    }
}
# ===================================================================

st.set_page_config(page_title="AI 商业选型系统", layout="wide")
st.title("🚀 AI 材料商业评估与选型系统 (v18.0 领域自适应版)")
...
with st.sidebar:
    ...
    st.header("2. 通用基础参数")
    mat_category = st.selectbox("材料所属大类", ["合成蛋白/生物基大分子", "可降解聚合物", "高性能碳基纤维", "特种合金"])
...
if st.button("🚀 启动专属领域全量评估", type="primary"):
    if not api_key: st.warning("请配置 DeepSeek API Key。"); st.stop()

    with st.spinner(f"正在切换为【{domain.split(' ')[0]}】专家模式，加载专属知识图谱..."):
        
        # 1. 靶向搜索：加上 search_suffix 后，搜索结果绝对不会跨界串味
        web_query = f"{mat_category} {strength}MPa {current_config['search_suffix']}"
        web_context = search_tavily(web_query, tavily_key)
        
        # 2. 本地检索
        local_query = f"{mat_category} 加工工艺 {domain}"
        rag_context = retrieve_knowledge(local_query)

    # 构建强约束提示词
    system_prompt = f"""
    你是全球顶尖的材料评估系统。你当前正在执行【{domain}】领域的专属评估。
    
    【输入基础参数】: 密度={density}, 强度={strength}, 模量={modulus}。
    【当前领域专属数据】: {json.dumps(domain_specific_data, ensure_ascii=False)}。你必须在报告中深度分析这些专属参数带来的影响！
    
    💥【领域安全隔离防线 (CRITICAL)】💥
    {current_config['strict_constraint']}
    如果违反上述隔离禁令，你的评估将被直接判定为不合格！
    ...
    JSON 格式严格如下：
    {{
      "radar": {{"核心强度": 100, "刚度支撑": 60, "轻量化收益": 95, "加工成型率": 45, "环境/生化抗性": 80, "领域专属适应性": 90}},
      "base_metrics": [
      ...
      "math_sim": {{
        "part_name": "核心部件名称(必须契合选定领域，例如医疗选'承力骨板')",
        "design_goal": "核心等效目标",
      ...
      "eight_dimensions": [
        ...
        {{"dim": "5. 【重点】领域专属生化/理化抗性", "details": ["结合专属参数深度剖析(如降解/阻燃/防弹等)", "数据论证2"], "chart_metric": "专属环境保持率", "base_val": 90, "new_val": 45}},
        {{"dim": "6. 【专属】材料微观机理对当前领域的影响", "details": ["剖析微观特性", "数据点2"], "chart_metric": "本征结构优越度", "base_val": 50, "new_val": 90}},
        {{"dim": "7. 全生命周期碳足迹与ESG", "details": ["评估制造碳排或降解环保性", "数据点2"], "chart_metric": "ESG综合减排贡献", "base_val": 20, "new_val": 85}},
      ...
      "case_study": {{
        ...
        "traditional_mat": "传统现役基准方案",
        "new_design": "新材料应用方案",
        "benefit": "商业量化总收益说明"
      }},

      "grand_verdict": {{
        "summary": "最终选型陈词：给出明确的投产或禁用建议。",
    ...
    with st.spinner("数据隔离与防交叉污染已开启，正在生成专属报告..."):
        try:
            ...
            st.success("✅ 商业级领域专属数据推演完成！")

            # =============== 以下为 UI 渲染逻辑 (保持极高的视觉标准不变) ===============
            ...
            with c_bars:
                ...
                def plot_mini(md, container):
                    ...
                    fig = px.bar(df_m, x="方案", y="数值", text_auto='.2s', color="方案", color_discrete_sequence=["#a6b8c7", "#5a6e7f", "#ff4b4b"])
            ...
            with sc2:
                df_wt = pd.DataFrame({"方案": ["传统基准", "新材料代换"], "终端效能": [sim['chart_vals']['base_wt'], sim['chart_vals']['new_wt']]})
                fig_wt = px.bar(df_wt, x="方案", y="终端效能", color="方案", text="终端效能", height=200, color_discrete_map={"传统基准": "#7f7f7f", "新材料代换": "#2ca02c"})
            ...
            with bot_c1:
                ...
                <div style="background: linear-gradient(135deg, #0f172a, #334155); border-radius: 10px; padding: 40px 20px; text-align: center; color: white; height: 260px; display: flex; flex-direction: column; justify-content: center; align-items: center; border: 2px dashed #475569; margin-bottom: 20px;">
                    <h2 style="margin-bottom: 10px; color: #e2e8f0; font-family: sans-serif;">⚙️ AI 拓扑渲染引擎</h2>
                    <p style="color: #94a3b8; font-size: 16px; margin: 0;">载入场景: {domain.split(' ')[0]}</p>
                </div>
            ...
            st.divider()
            st.subheader("📚 数据溯源 (Data Tracking)")
            for ref in data.get('reference_sources', []): st.markdown(f"- 🔗 `{ref}`")

        except Exception as e:
            st.error(f"后台节点过载，解析中断。错误追踪: {str(e)}")
